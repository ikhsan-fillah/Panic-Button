...
class _SOSBottomSheetState extends State<SOSBottomSheet>
  ...
  IconData _kategoriIcon(String nama) {
    ...
    if (key.contains('curi') || key.contains('keamanan')) return Icons.local_police_rounded;
  ...
  Color _kategoriColor(String nama) {
    final key = nama.toLowerCase();
    if (key.contains('curi') || key.contains('keamanan') || key.contains('kelahi')) {
      return AppTheme.danger;
    }
    if (key.contains('bakar') || key.contains('bencana')) return AppTheme.warning;
        ...
        child: Form(
          ...
          child: SingleChildScrollView(
            ...
            if (_selectedKategori >= 0 &&
                ...
                _isKategoriLainnya(_laporanCtrl.kategoriList[_selectedKategori]))
              Column(
                children: [
                  TextFormField(
              ...
              children: [
                ...
                Expanded(
                  ...
                  child: Obx(() => ElevatedButton(
                        onPressed: (_selectedKategori < 0 || _laporanCtrl.isSendingSOS.value)
                            ? null
                            : () {
                                if (!_formKey.currentState!.validate()) return;
                                final kategori = _laporanCtrl.kategoriList[_selectedKategori];
                                final kategoriId = int.tryParse(
                                      kategori['id'].toString(),
                                    ) ??
                                    0;
                                if (kategoriId == 0) return;
                                final kategoriNama = kategori['nama']?.toString() ?? 'Darurat';
                                final isLainnya = _isKategoriLainnya(kategori);
                                final judul = isLainnya
                                    ? _judulCtrl.text.trim()
                                    : kategoriNama;
                                _laporanCtrl.selectedPriority.value = _selectedPriority;
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            const SizedBox(width: 8),
                            Text(
                              _laporanCtrl.isSendingSOS.value ? 'Mengirim...' : 'Kirim Laporan',
